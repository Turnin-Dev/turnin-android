...
class ValidateNameUseCase {
    operator fun invoke(name: String): Flow<ValidationResult<RegisterValidationError>> = flow {
        ...
        when {
            ...
            name.matches(RegexPatterns.name) == false -> {
                emit(ValidationResult.Error(RegisterValidationError.ONLY_ALPHANUMERIC_HANGUL))
                return@flow
            }
        }

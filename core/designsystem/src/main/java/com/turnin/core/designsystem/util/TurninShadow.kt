...
/** TurninShadow 타입 */
sealed class TurninShadowType {
    data object Normal : TurninShadowType()
 ...
 * @param type 그림자 타입
 * @param shape 그림자 모양
 * @see TurninShadowType
 */
fun Modifier.turninShadow(
    type: TurninShadowType,
    shape: Shape = RectangleShape,
): Modifier = composed {
    val isDarkMode = isSystemInDarkTheme()

    when (type) {
        TurninShadowType.Normal -> {
            this.dropShadow(
                shape = shape,
                shadow = Shadow(
                    radius = 3.dp,

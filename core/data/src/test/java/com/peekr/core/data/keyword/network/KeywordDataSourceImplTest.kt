...
class KeywordDataSourceImplTest {
    ...
    @Test
    fun `키워드 ID로 키워드 조회 - 잘못된 응답 바디로 응답 시 알려진 에러를 반환한다`() = runTest {
        // given
        testRule.server.enqueue(
            MockResponse().apply {
                setResponseCode(200)
    ...
    @Test
    fun `키워드 ID로 키워드 조회 - 알 수 없는 예외 발생 시 Unexpected 에러를 반환한다`() = runTest {
        // given
    ...
    @Test
    fun `키워드 ID로 키워드 조회 - HTTP 상태코드 404 응답 시 NotFound 에러로 반환한다`() = runTest {
        // given
        testRule.server.enqueue(
            MockResponse().apply {
                setResponseCode(404)
    ...
    fun `키워드 명으로 키워드 조회 - 성공 테스트`() = runTest {
        ...
        // when
        val response = dataSource.getKeywordByName(TEST_KEYWORD)
        val expectedKeywordResponse =
            testRule.decodeFromJson<KeywordResponse>(TestKeywordResponse)
    ...
    @Test
    fun `키워드 명으로로 키워드 조회 - 잘못된 응답 바디로 응답 시 알려진 에러를 반환한다`() = runTest {
        // given
        testRule.server.enqueue(
            MockResponse().apply {
                setResponseCode(200)
    ...
    @Test
    fun `키워드 명으로 키워드 조회 - 알 수 없는 예외 발생 시 Unexpected 에러를 반환한다`() = runTest {
        // given
        val mockApi: KeywordApi = mockk()
        val exception = Exception()
        dataSource = KeywordNetworkDataSource(mockApi)
    ...
    @Test
    fun `키워드 명으로 키워드 조회 - HTTP 상태코드 404 응답 시 NotFound 에러로 반환한다`() = runTest {
        // given
        testRule.server.enqueue(
            MockResponse().apply {
                setResponseCode(404)

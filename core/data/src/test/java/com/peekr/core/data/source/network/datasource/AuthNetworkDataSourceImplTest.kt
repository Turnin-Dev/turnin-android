...
class AuthNetworkDataSourceImplTest {
    ...
    fun `login() 성공 테스트`() = runTest {
        ...
        Assert.assertEquals((result as NetworkResult.Success).data, mockLoginResponse)
    }
    ...
    fun `login() 실패 테스트 (잘못된 응답 바디)`() = runTest {
        ...
        Assert.assertEquals((result as NetworkResult.Error).error, NetworkErrorType.Exception.JsonData)
    }
    ...
    fun `login() 실패 테스트 (알 수 없는 API 예외 발생)`() = runTest {
        ...
        // then
        Assert.assertTrue(result is NetworkResult.Error)
        Assert.assertEquals((result as NetworkResult.Error).error, NetworkErrorType.Unexpected(exception))
    }
    ...
    fun `existsUser() 성공 테스트`() = runTest {
        ...
        // then
        Assert.assertTrue(result is NetworkResult.Success)
        Assert.assertTrue((result as NetworkResult.Success).data.exists)
    }
    ...
    fun `register() 성공 테스트`() = runTest {
        ...
        // then
        Assert.assertTrue(result is NetworkResult.Success)
        Assert.assertEquals(mockRegisterResponse, (result as NetworkResult.Success).data)
    }

...
class TurninPagingSourceTest {
    private val mockApiCall: ApiCallMock = mockk()

    private fun createPagingSource(): TurninPagingSource<Item, ItemListResponse> =
        TurninPagingSource(
    ...
    fun `최초 로드 (Initial Load) 성공 테스트`() = runTest {
        ...
        val expectedResult = LoadResult.Page(
            data = items,
            prevKey = null,
            nextKey = TurninPagingSource.START_PAGE_INDEX + 1,
        )

        // 1(초기) 페이지 요청 시 성공 응답을 반환하도록 설정
        coEvery {
            mockApiCall.call(TurninPagingSource.START_PAGE_INDEX)
